  ...
  return (
    <section id="home" className="relative flex min-h-[100svh] items-center overflow-hidden pt-[76px]">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-grid-steel [background-size:60px_60px] opacity-40" />
      </div>
      ...
      <div className="container-x relative z-10 py-20">
        <div className="max-w-3xl">
          ...
          <motion.p
            className="mt-7 max-w-xl text-lg leading-relaxed text-steel-300"
        ...
        {/* stat strip */}
        <motion.div
          className="mt-16 grid max-w-3xl grid-cols-2 gap-px overflow-hidden rounded-lg border border-white/10 bg-white/5 backdrop-blur sm:grid-cols-4"
          initial={{ opacity: 0, y: 24 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, delay: 0.85 }}
        >
          {STATS.map((s) => (
            <div key={s.label} className="bg-navy-950/40 px-5 py-5">
              <div className="font-display text-3xl text-white">{s.value}</div>
              <div className="mt-1 text-xs font-medium uppercase tracking-wide text-steel-400">{s.label}</div>
            </div>
          ))}
        </motion.div>
      </div>
