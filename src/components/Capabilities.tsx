...
export default function Capabilities() {
  return (
    <section id="capabilities" className="relative border-y border-white/5 bg-graphite-950 py-24 lg:py-32">
          ...
          {CAPABILITIES.map((c, i) => (
            <Reveal as="article" key={c.title} delay={(i % 4) * 0.06}>
              <div className="card-surface group h-full p-6 hover:-translate-y-1 hover:border-copper-500/40 hover:shadow-card">
                <div className="mb-5 inline-flex h-14 w-14 items-center justify-center rounded-lg border border-white/10 bg-gradient-to-br from-navy-800 to-graphite-900 text-copper-400 transition-colors group-hover:border-copper-500/50 group-hover:text-copper-300">
                  <c.icon className="h-7 w-7" />
                </div>
                <h3 className="text-lg text-white">{c.title}</h3>
                <p className="mt-2.5 text-sm leading-relaxed text-steel-400">{c.desc}</p>
