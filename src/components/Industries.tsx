...
export default function Industries() {
  return (
    <section id="industries" className="relative bg-white py-24 lg:py-32">
          ...
          {INDUSTRIES.map((ind, i) => (
            <Reveal as="article" key={ind.title} delay={(i % 4) * 0.06}>
              <div className="group relative overflow-hidden rounded-lg border border-line">
                ...
                <div className="pointer-events-none absolute left-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-md border border-line bg-navy-950/50 text-copper-400 backdrop-blur-sm">
                  <ind.icon className="h-5 w-5" />
                </div>
                <div className="absolute inset-x-0 bottom-0 p-5">
                  <h3 className="text-xl text-white drop-shadow">{ind.title}</h3>
                  <p className="mt-1 max-h-0 overflow-hidden text-sm text-white/85 opacity-0 transition-all duration-300 group-hover:max-h-16 group-hover:opacity-100">
                    {ind.desc}
                  </p>
                </div>
                <span className="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-full border border-line bg-black/30 text-white opacity-0 transition-all duration-300 group-hover:opacity-100">
